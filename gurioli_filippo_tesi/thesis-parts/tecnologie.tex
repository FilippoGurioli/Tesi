...
confronti tra framework simili motivando la scelta intrapresa per questo progetto, analizzando i vari aspetti che hanno portato a questa decisione.
...
\begin{itemize}
    ...
    \item \textit{rendering system}, utilizzato nel rendering della scena virtuale, sfruttando funzioni avanzate come il \textit{clipping}, \textit{pulse shaders} e \textit{materials}.
\end{itemize}
...
l'infrastruttura di Unity dei prefab, dei componenti e del game loop, questi però non verranno approfonditi per non allontanarsi troppo dallo scopo di questo lavoro. Essendo 
...
l'accesso alle pagine che la utilizzano è permesso solo tramite HTTPS.
...
\begin{itemize}
    ...
    modello di gestione degli eventi asincrono. Questo permette di gestire un elevato numero di richieste senza dover creare un thread per ogni richiesta, ma gestendo ogni richiesta
    come un evento asincrono. Questa tecnica è anche utilizzata per la gestione degli I/O in modo tale da non bloccare l'unico thread disponibile.
...
BabylonJS è un framework open source per la creazione di applicazioni web 3D e XR. Questo framework è stato sviluppato per essere utilizzato con NodeJS e permette di creare
applicazioni web in modo semplice e veloce\cite{BabylonJS}.\\
...
\subsection{Motivazioni e principali concorrenti}
...
ancora e incorporando anche molte funzionalità come il \textit{followBehaviour}, l'\textit{eye tracking} e l'\textit{hand tracking}.\\
...
comportato non poche difficoltà nell'integrazione con il resto della codebase. In aggiunta a questo, Unity non supporta nativamente WebXR, seppur esistano plug in che permettono
